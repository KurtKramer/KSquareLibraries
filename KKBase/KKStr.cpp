...
inline int32  KKB::STRICMP (const char*  left,
                            const char*  right
                           )
{
  if  (left == NULL)
        ...
        val (NULL)
{
  ...
  uint32  newLen = (uint32)strlen (str);
...
int32  KKStr::CountInstancesOf (char  ch)  const
{
  if  (!val)
    return 0;
  int32  count = 0;
...
void  KKStr::Append (const char* buff)
{
  ...
  uint32  buffLen = (uint32)strlen (buff);
...
int32  KKStr::LocateNthOccurrence (char ch,  int32 n)  const
{
  if  (!val)
    return -1;
...
KKB::uint32  KKStr::ExtractTokenUint (const char* delStr)
{
  #ifdef  KKDEBUG
  ValidateLen ();
  #endif
...
KKB::uint64  KKStr::ExtractTokenUint64 (const char* delStr)
{
  #ifdef  KKDEBUG
  ValidateLen ();
  #endif
...
KKB::uint64  KKStr::ToUint64 () const
{
  if  (!val)  return 0;
  #if  defined(__GNUC__)
...
void  KKStr::MemSet (void* dest,  uint8  byte, uint32  size)
{
  if  (dest == NULL)
  {
...
KKStr  KKB::StrFormatInt (int32        val,
                          const char*  mask
                         )
{
...
KKStr  KKB::StrFormatInt64 (int64        val,
                            const char*  mask
                           )
{
  // Get num of decimal Places
...
KKStr  KKB::StrFromUint16 (uint16 ui)
{
  char  buff[50];
...
KKStr  KKB::StrFromInt32 (int32 i)
{
  char  buff[50];
...
KKStr  KKB::StrFromUint32 (uint32 ui)
{
  char  buff[50];
...
KKStr  KKB::StrFromInt64 (int64 i64)
{
  char  buff[50];
...
KKStr  KKB::StrFromUint64 (uint64 ul)
{
  char  buff[50];
...
const  uint32  KKB::KKStr::StrIntMax = USHRT_MAX;
