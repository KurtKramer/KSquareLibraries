...
{
  void  sigmoid_train (int32          numExamples, 
  ...
  void  multiclass_probability (int32     numClasses,     /**< Number of Classes.      */
...
public:
...

  {
    ...
    cache = new Cache (prob.numTrainExamples, 
                       (int32)(param.cache_size * (1 << 20))
                      );
    QD = new Qfloat[prob.numTrainExamples];

    for  (int32 i = 0;  i < prob.numTrainExamples;  i++)
...
public:
  ...
  {
    cache = new Cache (prob.numTrainExamples, (int32)(param.cache_size * (1<<20)));
    QD = new Qfloat[prob.numTrainExamples];
    for  (int32 i = 0;  i < prob.numTrainExamples;  i++)
      QD[i]= (Qfloat)(this->*kernel_function)(i, i);
  }
                              ...
                              )
{
  ...
  int32    numTrainExamples = prob->numTrainExamples;
                                  ...
                                  )
{
  ...
  int32 n = (int32)(param->nu * prob->numTrainExamples);  // # of alpha's at upper bound
...
void  SVM289_MFS::sigmoid_train (int32          numExamples, 
                                 const double*  dec_values, 
                                 const double*  labels, 
                                 double&        A,
                                 double&        B
...
void  SVM289_MFS::multiclass_probability (int32     numClasses,     /**< Number of Classes.      */
                                          double**  pairwiseProbs,  /**< Pairwise Probabilites.  */
                                          double*   classProb       /**< Class Probability       */
                                         )
{
  int32 t,j;
  int32  iter     = 0;
  int32  max_iter = Max (100, numClasses);
                                 ...
                                 )
{
  int32 i;
  int32  nr_fold = 5;
  int32 *perm = new int32[prob->numTrainExamples];
  ...
  for  (i = 0;  i < prob->numTrainExamples;  i++)
  {
    int32 j = i + rand() % (prob->numTrainExamples-i);
    SVM289_MFS::swap (perm[i], perm[j]);
  }
    
  for  (i = 0;  i < nr_fold;  i++)
  {
    int32 begin = i * prob->numTrainExamples / nr_fold;
    int32 end   = (i + 1) * prob->numTrainExamples / nr_fold;
    int32 j, k;

    int32  subL = prob->numTrainExamples - (end - begin);
                                       ...
                                       )
{
  ...
  int32 numTrainExamples = prob.numTrainExamples;
  int32 *perm = new int32[numTrainExamples];
  int32 nr_class;
      ...
      )
  {
    ...
    int32 c;
    int32 *index = new int32[numTrainExamples];
    for  (i = 0;  i < numTrainExamples;  i++)
      index[i] = perm[i];
  ...
  else
  {
    ...
    for (i = 0;  i < numTrainExamples;  i++)
    {
      int32 j = i + rand() % (numTrainExamples - i);
      SVM289_MFS::swap (perm[i], perm[j]);
    }
    for  (i = 0;  i <=  nr_fold; i++)
                                     ...
                                     )
{
      ...
      )
  {
    ...
    double sum = 0;
    for  (int32 i = 0;  i < model->numSVs;  i++)
