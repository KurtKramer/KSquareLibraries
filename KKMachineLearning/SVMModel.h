...
public:
  ...
  /**
   *@brief Loads an SVM model from disk.
   *@details  The '_cancelFlag' parameter is meant to allow another thread to cancel procesing 
   *          by a diiferent thread in SVMmodel.  Ex:  If one thread is building a new SVM for
...
private:
   ...
   */
  int32  EncodeExample (FeatureVectorPtr  example,
                        svm_node*         row
                       );
  ...
  MLClassPtr*         classIdxTable;         /**< Supports reverse class lookUp,  indexed by ClassAssignments num,
